...
if ( ! function_exists( 'multistore_get_lowest_price' ) ) {
	/**
	 * Get lowest price data for a product
	 *
	 * @since 1.0.0
	 * @param int $product_id Product ID (default: current product).
	 * @return array|null Lowest price data or null.
	 */
	function multistore_get_lowest_price( int $product_id = 0 ): ?array {
		if ( ! $product_id ) {
			$product_id = get_the_ID();
		}

		return \MultiStore\Plugin\Utils\Price_History_Helpers::get_lowest_price( $product_id );
	}
}

if ( ! function_exists( 'multistore_display_lowest_price' ) ) {
	/**
	 * Display lowest price info
	 *
	 * @since 1.0.0
	 ...
	 * @param string $format     Format: 'full', 'price', 'simple'.
	 */
	function multistore_display_lowest_price( int $product_id = 0, string $format = 'full' ): void {
		if ( ! $product_id ) {
			$product_id = get_the_ID();
		}

		\MultiStore\Plugin\Utils\Price_History_Helpers::display_lowest_price( $product_id, $format );
	}
}

if ( ! function_exists( 'multistore_has_lowest_price' ) ) {
	/**
	 * Check if product has lowest price info
	 *
	 * @since 1.0.0
	 ...
	 * @return bool True if has info, false otherwise.
	 */
	function multistore_has_lowest_price( int $product_id = 0 ): bool {
		if ( ! $product_id ) {
			$product_id = get_the_ID();
		}

		return \MultiStore\Plugin\Utils\Price_History_Helpers::has_lowest_price( $product_id );
	}
}

if ( ! function_exists( 'multistore_get_price_history' ) ) {
	/**
	 * Get price history for a product
	 *
	 * @since 1.0.0
	 ...
	 * @return array Array of price records.
	 */
	function multistore_get_price_history( int $product_id = 0, int $days = 30 ): array {
		if ( ! $product_id ) {
			$product_id = get_the_ID();
		}

		return \MultiStore\Plugin\Utils\Price_History_Helpers::get_price_history( $product_id, $days );
	}
}
